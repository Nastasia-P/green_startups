...
`deal_investors_clean`). No transformation happens here beyond the two join helpers
that attach green / firm attributes to the relation and deal grains.
"""
...
def load_firm_table(firm_table_path: Path | None = None) -> pd.DataFrame:
    ...
    log(f"[step6] load firm table: {path} rows={len(df)}")
    return df
...
def _load_clean(name: str, clean_dir: Path | None = None) -> pd.DataFrame:
    ...
    if not path.exists():
        log(f"[step6] load {name}: {path} MISSING -> empty")
        return pd.DataFrame()
    df = pd.read_parquet(path)
    log(f"[step6] load {name}: {path} rows={len(df)}")
    return df
