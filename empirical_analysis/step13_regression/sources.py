"""Input access for Step 13 (exploratory regression).

Loads the Step 2 firm table and, for the family-3 fixed-horizon capital outcome,
the Step 1 `deals_clean` parquet. No transformation happens here.
"""
...
def load_firm_table(firm_table_path: Path | None = None) -> pd.DataFrame:
    ...
    log(f"[step13] load firm table: {path} rows={len(df)}")
    return df
...
def load_deals_clean(clean_dir: Path | None = None) -> pd.DataFrame:
    ...
    if not path.exists():
        log(f"[step13] load deals_clean: {path} MISSING -> empty")
        return pd.DataFrame()
    df = pd.read_parquet(path)
    log(f"[step13] load deals_clean: {path} rows={len(df)}")
    return df
