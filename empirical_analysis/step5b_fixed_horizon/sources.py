"""Input access for Step 5b (fixed five-year horizon).

Loads the Step 2 firm table and the Step 1 clean tables used here
(`deals_clean`, `deal_investors_clean`, `investors_clean`). No transformation
happens here; all windowing lives in `build.py`. Missing clean tables return
empty frames so the module degrades gracefully rather than crashing.
"""
...
def load_firm_table(firm_table_path: Path | None = None) -> pd.DataFrame:
    ...
    log(f"[step5b] load firm table: {path} rows={len(df)}")
    return df
...
def _load_clean(name: str, clean_dir: Path | None = None) -> pd.DataFrame:
    ...
    if not path.exists():
        log(f"[step5b] load {name}: {path} MISSING -> empty")
        return pd.DataFrame()
    df = pd.read_parquet(path)
    log(f"[step5b] load {name}: {path} rows={len(df)}")
    return df
